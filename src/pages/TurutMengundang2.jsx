export default function TurutMengundang2() {
  return (
    <div className="midlePosition">
      <p>
        Atas Kehadiran dan Do’a Restu Bapak/Ibu/Saudara (i),
        <br />
        kami ucapka terima kasih <br />
        Wassalamu Alaikum Warah,matullahi Wabarakatuh <br />
        Hormat Kami yang mengundang :
      </p>
      <h3 style={{ fontWeight: 900, marginTop: 0 }}>
        H. HASAN DG. TALLI / HJ. ROSTINA DG. NGUGI
      </h3>
      <h3 style={{ fontWeight: 900, marginTop: 0 }}>
        SULFAN LUKMAN, S.E / MUSLIHATI HASAN, ST., M.T
      </h3>
      <h3 style={{ fontWeight: 900, marginTop: 0 }}>
        MUH. SAFAR HAMID, S.T / JUMRA HASAN, S.T
      </h3>
      <p>Turut Mengundang</p>
      <div className="namaTurut">
        <h3 style={{ fontWeight: 900, textAlign: "left" }}>
          *Aqsa Ilyas Dg. Sibali <br />
          *Hj. Dahlia Dg. Ratu <br />
          *Iskandar Dg. Romo <br />
          *Taufik, A.mD Dg. Nanring
          <br />
          *Muh. Iqbal Ilyas, S.e Dg. Magassing <br />
          *Nurfalah Mizeldi Hasan
        </h3>
        <h3 style={{ fontWeight: 900, textAlign: "left" }}>
          *Molle Dg. Rangka
          <br />
          *AKP. Mustafani Dg. Ngewa
          <br />
          *Dahlan Dg. Pawero
          <br />
          *Hasrun, S.Kes Dg. Situju
          <br />
          *Rahman Dg. Lulung
          <br />
          *Habir Dg. Paturu
        </h3>
      </div>
    </div>
  );
}
